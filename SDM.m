...
% Number of panels
cp = length(XB)-1;                             % Number of panels (control points)

%% CHECK PANEL DIRECTIONS - FLIP IF NECESSARY

% Check for direction of points
edge = zeros(cp,1);                                   % Initialize cp
for i = 1:1:cp                                        % Loop over all panels
...
XC   = zeros(cp,1);                                                     % Initialize control point X-coordinate array
YC   = zeros(cp,1);                                                     % Initialize control point Y-coordinate array
L    = zeros(cp,1);                                                     % Initialize panel length array
phiD = zeros(cp,1);                                                     % Initialize panel orientation angle array [deg]
...
% - Refs [2] and [3]
[I,J] = COMPUTE_SDM(XC,YC,XB,YB,phi,L);                                  % Compute geometric integrals

% Populate A matrix
% - Simpler option: A = I + pi*eye(cp,cp);
A = zeros(cp,cp);                                                   % Initialize the A matrix
for i = 1:1:cp                                                          % Loop over all i panels
    for j = 1:1:cp                                                      % Loop over all j panels
...
b = zeros(cp,1);                                                        % Initialize the b array
for i = 1:1:cp                                                          % Loop over all panels
...
%                   Cop = 1-(Vt/Uo).^2;
Vt = zeros(cp,1);                                                       % Initialize tangential velocity array
Cop = zeros(cp,1);                                                       % Initialize pressure coefficient array
for i = 1:1:cp                                                         % Loop over all i panels
    ...
    Cop(i) = 1-(Vt(i)/Uo)^2;                                               % Compute pressure coefficient
end

% Analytical angles and pressure coefficients
analyticTheta = linspace(0,2*pi,200)';                                      % Analytical theta angles [rad]
analyticCoP    = 1-4*sin(analyticTheta).^2;                                  % Analytical pressure coefficient []

%% COMPUTE LIFT AND DRAG

% Compute normal and axial force coefficients
CN = -Cop.*L.*sin(beta);                                                      % Normal force coefficient []
CA = -Cop.*L.*cos(beta);                                                      % Axial force coefficient []
...
if (flagPlot(4) == 1 || flagPlot(5) == 1)
    ...
    for m = 1:1:nGridX                                                      % Loop over X-grid points
        for n = 1:1:nGridY                                                  % Loop over Y-grid points
            ...
            [Mx,My] = STREAMLINE_SPM(XP,YP,XB,YB,phi,L);                    % Compute streamline Mx and My values (Ref [4])
    ...
    CopXY = 1-(Vxy./Uo).^2;                                                % Pressure coefficient []
end
...
if (flagPlot(1) == 1)
    ...
    for i = 1:1:cp                                                      % Loop over all panels
        ...
        X(2) = XC(i) + L(i)*cosd(betaD(i)+AoA);                             % Set X end of panel orientation vector
        Y(1) = YC(i);                                                       % Set Y start of panel orientation vector
        Y(2) = YC(i) + L(i)*sind(betaD(i)+AoA);                             % Set Y end of panel orientation vector
...
if (flagPlot(3) == 1)
    ...
    set(gcf,'Color','White');                                               % Set color to white
    set(gca,'FontSize',12);                                                 % Set font size
    pA = plot(analyticTheta,analyticCoP,'k-','LineWidth',3);                 % Plot analytical pressure coefficient
    pC = plot(beta,Cop,'ks','MarkerFaceColor','r','MarkerSize',10);          % Plot compute pressure coefficient
    xlabel('Angle [rad]');                                                  % Set X-label
    ylabel('Cop');                                                           % Set Y-label
...
if (flagPlot(5) == 1)
    ...
    xlabel('X Units');                                                      % Set X-label
    ylabel('Y Units');                                                      % Set Y-label
    contourf(XX,YY,CopXY,100,'EdgeColor','none');                            % Plot contour
    fill(XB,YB,'k');                                                        % Plot polygon
    xlim(xVals);                                                            % Set X-limits
    ylim(yVals);                                                            % Set Y-limits
    zoom reset;                                                             % Reset zoom
